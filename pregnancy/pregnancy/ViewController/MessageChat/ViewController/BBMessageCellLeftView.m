...
@implementation BBMessageCellLeftView
@synthesize dateLabel,avtarButton,deleteButton,contentLabel,contentLabelBg,data,selectStatusValue,messageChatDelegate,linkButton;
...
-(void)setData:(NSDictionary *)commentData withEditStatus:(BOOL)editStatus withSelectStatus:(BOOL)selectStatus withBBMessageChatDelegate:(id<BBMessageChatDelegate>)delegate{
	...
	CGSize size =
    [BBAutoCalculationSize autoCalculationSizeRect:CGSizeMake(MAXFLOAT, 13.0f) withFont:[UIFont systemFontOfSize:13.0] withString:contentStl];
    if (size.width<=156) {
        if (size.width<=45) {
            width = 45;
        }else
        ...
        size =
            [BBAutoCalculationSize autoCalculationSizeRect:CGSizeMake(156.0f, MAXFLOAT) withFont:[UIFont systemFontOfSize:13.0] withString:content];
        width = 156;
        if (size.height>42) {
            height = size.height;
        }else{
    ...
    if (editStatus) {
        ...
        self.contentLabel.frame=CGRectMake(137, 19, width, height);
       [self.avtarButton setFrame:CGRectMake(60, 16, self.avtarButton.frame.size.width, self.avtarButton.frame.size.height)];
        ...
        self.contentLabel.frame=CGRectMake(77, 19, width, height);
        [self.avtarButton setFrame:CGRectMake(7, 16, self.avtarButton.frame.size.width, self.avtarButton.frame.size.height)];
        [self.deleteButton setFrame:CGRectMake(self.deleteButton.frame.origin.x, 16+avtarButtonOffsetY, self.deleteButton.frame.size.width, self.deleteButton.frame.size.height)];
    }
    ...
    UIImage *image = nil;
    image = [[UIImage imageNamed:@"message_other_bg"] stretchableImageWithLeftCapWidth:20 topCapHeight:34];
    if (height>42) {
        [self.contentLabelBg setFrame:CGRectMake(contentLabel.frame.origin.x-14, contentLabel.frame.origin.y-4, contentLabel.frame.size.width+32, contentLabel.frame.size.height+8)];
    }else{
        [self.contentLabelBg setFrame:CGRectMake(contentLabel.frame.origin.x-14, contentLabel.frame.origin.y, contentLabel.frame.size.width+24, contentLabel.frame.size.height)];
    }
    ...
    CGFloat cellHeight = [BBMessageCellLeftView calculateHeightWithContent:commentData];
    ...
    if ([[data stringForKey:@"type"] isEqualToString:@"1"] ||[[data stringForKey:@"type"] isEqualToString:@"2"] || [[data stringForKey:@"type"] isEqualToString:@"3"])
    {
        ...
        if([[data stringForKey:@"type"] isEqualToString:@"1"] ||[[data stringForKey:@"type"] isEqualToString:@"2"] )
        {
            [self.linkButton setImage:[UIImage imageNamed:@"message_xiangqing_link"] forState:UIControlStateNormal];
            [self.linkButton setImage:[UIImage imageNamed:@"message_xiangqing_link_pressed"] forState:UIControlStateHighlighted];
        }
        else if([[data stringForKey:@"type"] isEqualToString:@"3"])
        {
            [self.linkButton setImage:[UIImage imageNamed:@"message_baoxi_link"] forState:UIControlStateNormal];
            [self.linkButton setImage:[UIImage imageNamed:@"message_baoxi_link_pressed"] forState:UIControlStateHighlighted];
        }
    }
    else
    {
