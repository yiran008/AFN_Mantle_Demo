    ...
    while (found)
    {
            ...
            if (aDownloader != downloader)
        {
            ...
            if (image)
            {
                ...
                //add By zhenglei，需要返回图片的data
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithData:forURL:userInfo:)])
                {
                    NSDictionary *userInfo = [info objectForKey:@"userInfo"];
                    if ([userInfo isKindOfClass:NSNull.class])
                    {
                    ...
                    ((void ( *)(id, SEL, id, id, id, id))objc_msgSend)(delegate, @selector(webImageManager:didFinishWithData:forURL:userInfo:), self, downloader.imageData, downloader.url, userInfo);
                }
                else
                // add end
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
