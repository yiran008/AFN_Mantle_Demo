...
- (void)layoutSubviews
{
    ...
    {
        UIView * view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.bounds.size.height - 3)];
        view.backgroundColor = [UIColor whiteColor];
        UIView * line = [[UIView alloc]initWithFrame:CGRectMake(0,view.frame.size.height-0.5,self.frame.size.width, 0.5)];
        line.backgroundColor = [UIColor colorWithRed:0.3 green:0.3 blue:0.3 alpha:0.2];
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionIndicatorLocation == HMSegmentedControlSelectionIndicatorLocationDown)
        indicatorYOffset = self.bounds.size.height - self.selectionIndicatorHeight -3;
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe && sectionWidth <= self.segmentWidth) {
        ...
        return CGRectMake(x + sectionWidth * 1./6., indicatorYOffset, sectionWidth * 4./6., self.selectionIndicatorHeight);
    } else {
        return CGRectMake(self.segmentWidth * self.selectedSegmentIndex + self.segmentWidth* 1./10., indicatorYOffset, self.segmentWidth * 4./5., self.selectionIndicatorHeight);
    }
}
