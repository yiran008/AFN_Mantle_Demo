...
//        _photo.srcImageView.image = nil;
        ...
        if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
            ...
            [_imageView setImageWithURL:_photo.url placeholderImage:_photo.placeholder options:SDWebImageRetryFailed|SDWebImageLowPriority success:^(UIImage *image, BOOL cached) {
                photo.image = image;
                // 调整frame参数
                ...
                [photoView photoDidFinishLoadWithImage:nil];
            }];
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        [_imageView setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image != nil ? _photo.srcImageView.image:[UIImage imageNamed:@"topicPictureDefault"] options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSUInteger receivedSize, long long expectedSize)
         {
...
- (void)adjustFrame
{
    ...
    CGRect imageFrame = CGRectMake(0, 0, imageWidth * minScale, imageHeight * minScale);
    // 内容尺寸
    self.contentSize = CGSizeMake(0, imageFrame.size.height);
    
    // x值
    if (self.contentSize.width < boundsWidth) {
...
-(void)scrollViewDidZoom:(UIScrollView *)scrollView
{
    ...
    CGRect imageFrame = CGRectMake(0, 0, _imageView.width, _imageView.height);
    CGFloat boundsWidth = self.bounds.size.width;
    CGFloat boundsHeight = self.bounds.size.height;
    // y值
    if (self.contentSize.height < boundsHeight) {
        imageFrame.origin.y = floorf((boundsHeight - self.contentSize.height) / 2.0);
	} else {
        imageFrame.origin.y = 0;
	}

    if (self.contentSize.width < boundsWidth) {
        imageFrame.origin.x = floorf((boundsWidth - self.contentSize.width) / 2.0);
	} else {
        imageFrame.origin.x = 0;
	}
...
- (void)hide
{
    ...
    if ([self.photoViewDelegate respondsToSelector:@selector(photoViewWillDisappear)]) {
        [self.photoViewDelegate photoViewWillDisappear];
    }
