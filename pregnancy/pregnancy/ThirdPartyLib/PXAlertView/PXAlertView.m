...
//        self.alertView.backgroundColor = [UIColor colorWithWhite:0.25 alpha:1];
        self.alertView.backgroundColor = UI_POPVIEW_BGCOLOR;
...
//        self.titleLabel.textColor = [UIColor whiteColor];
        self.titleLabel.textColor = UI_TEXT_TITLE_COLOR;
...
//        self.messageLabel.textColor = [UIColor whiteColor];
        self.messageLabel.textColor = UI_TEXT_TITLE_COLOR;
...
- (void)setBackgroundColorForButton:(UIButton *)sender
{
    ...
    [sender setBackgroundColor:[UIColor colorWithHex:0xAAAAAA]];
}
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    if (self.completion) {
        ...
        // add by DJ
        if (sender != self.tap && self.buttons) {
...
//    lineLayer.backgroundColor = [[UIColor colorWithWhite:0.90 alpha:0.3] CGColor];
    lineLayer.backgroundColor = [[UIColor colorWithHex:0xBBBBBB] CGColor];
    return lineLayer;
}
...
{
    // add by DJ
    if ([[PXAlertViewStack sharedInstance] getAlertViewCount] >= 2)
    {
        return nil;
    }
...
- (void)closeAllAlertViews
{
    ...
    while (last)
    {
        [self closeAlertView:last];
        last = [self.alertViews lastObject];
    }
}

- (void)closeAlertView:(PXAlertView *)alertView
{
    [alertView dismiss:alertView.cancelButton];
    [alertView.view removeFromSuperview];
    [self pop:alertView];
}

- (NSInteger)getAlertViewCount
{
