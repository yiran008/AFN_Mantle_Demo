    ...
    for (NSNumber *type in types) {
        ...
        [self.assetsLibrary enumerateGroupsWithTypes:[type unsignedIntegerValue]
                                          usingBlock:^(ALAssetsGroup *assetsGroup, BOOL *stop) {
                                              ...
                                              if (!s_AlertView)
                                              {
                                                  if (error.code <= ALAssetsLibraryDataUnavailableError)
                                                  {
                                                      NSString *message;

                                                      if (IOS_VERSION >= 6.0)
                                                      {
                                                          message = @"~您可到\"设置-隐私-照片\"打开权限";
                                                      }
                                                      else
                                                      {
                                                          message = @"~您可到\"设置-定位服务\"打开权限";
                                                      }

                                                      s_AlertView = [PXAlertView showAlertWithTitle:@"宝宝树孕育没有权限访问您的照片" message:message cancelTitle:@"确定" otherTitle:nil completion:nil];
                                                  }
                                                  else
                                                  {
                                                      s_AlertView = [PXAlertView showAlertWithTitle:[error localizedDescription] message:nil cancelTitle:@"确定" otherTitle:nil completion:nil];
                                                  }
                                              }
                                              // add end
                                          }];
