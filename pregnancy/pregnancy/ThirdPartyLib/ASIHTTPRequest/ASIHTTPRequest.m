...
- (id)initWithURL:(NSURL *)newURL
{
    ...
    self.pregnancyMap = [[[BBPregnancyMap alloc] init] autorelease];
	return self;
}
...
- (void)setGetValue:(NSString *)value forKey:(NSString *)key
{
    ...
    NSString *path = [[[self url] absoluteString] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    if ([path rangeOfString:[NSString stringWithFormat:@"%@=%@", key, value]].location != NSNotFound)
    {
        return;
    }
    
    // 将get请求的参数保存到自定义字典中
    [self.pregnancyMap addPregnancyMapValue:value forKey:key];
    
    NSString *urlString = [path stringByAppendingFormat:[path rangeOfString:@"?"].location == NSNotFound ? @"?%@=%@" : @"&%@=%@", key, value];
    
    // url中已经存在key=键值对，需要修改key对应的value赋值为新的value
    NSArray *urlArray = [path componentsSeparatedByString:@"?"];
    if ([urlArray count] > 1)
    {
        NSString *pregnancyMap = [urlArray objectAtIndex:1];
        if (pregnancyMap)
        {
            NSArray *pregnancyMapArray = [pregnancyMap componentsSeparatedByString:@"&"];
            for (NSString *parameter in pregnancyMapArray)
            {
                if([parameter rangeOfString:[NSString stringWithFormat:@"%@",key]].location != NSNotFound)
                {
                    NSArray *valueArray = [parameter componentsSeparatedByString:@"="];
                    if([valueArray count] > 1)
                    {
                        urlString = [path stringByReplacingOccurrencesOfString:parameter withString:[NSString stringWithFormat:@"%@=%@", key, value]];
                    }
                }
            }
        }
    ...
    NSURL *newUrl = [NSURL URLWithString:[urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    
    [self setURL:newUrl];
}
//检查是否需要增加秘钥，需要则增加，否则则不加 by lijie
- (void)checkNeedAddSecret
{
    NSString *path = [[[self url] absoluteString] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    if ([path rangeOfString:@".babytree"].location != NSNotFound)
    {
        NSTimeInterval time = [[NSDate date] timeIntervalSince1970];
        [self setGetValue:[NSString stringWithFormat:@"%d", (int)time] forKey:@"local_ts"];
        NSString *md5 = [self.pregnancyMap sortDictOrderbyAlphabet];
        [self setGetValue:md5 forKey:@"secret"];
    }
}
