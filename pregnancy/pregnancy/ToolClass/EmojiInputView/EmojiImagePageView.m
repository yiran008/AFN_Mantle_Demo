...
- (UIFont *)emojiFont:(CGFloat)size
{
    UIFont* emojiFont = [UIFont fontWithName:@"AppleColorEmoji" size:size];
    return emojiFont;
}
...
- (void)drawPageAtIndex:(NSInteger)index
{
    ...
    //row number
	for (NSInteger i = 0; i < EMOJI_LINE; i++)
    {
		//column numer
		for (NSInteger j = 0; j < EMOJI_ONELINE_COUNT; j++)
        {
            ...
            button.titleLabel.font = [self emojiFont:30.0f];
